...
<script type="text/javascript">
			...
			<?php } else { 
				...
				if(!$_POST['submit']) {
					...
					else if(preg_match('#bilutv.com/(.*?)#s', $page)) $grabtype = 'bilu';
					...
					else if(preg_match('#moveek.com/(.*?)#s', $page)) $grabtype = 'move';
						...
						if ($title_en) {
							...
							if($check) {
								...
								$filmis = $film;	
										...
										if($title && $filmid == '0') {
											MySql::dbinsert("film","title,title_en,director,actor,category,country,duration,year,thumb,filmlb,quality,trailer,big_image,release_time,timeupdate,userpost,seotitle,thuyetminh","'$title','$title_en','$director','$actor','$category','$country','$duration','$year','$thumb','$filmlb','$quality','$trailer','$big_image','$release_time','$timeupdate','$userpost','$seotitle','$thuyetminh'");
