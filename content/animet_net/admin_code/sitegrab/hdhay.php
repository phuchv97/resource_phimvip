...
elseif ($quocgia == "Khác") $countryid = 8;
	...
	$category = $thtml;
## tập phim
preg_match('/<a id="btn-film-watch" class="btn btn-red" title="(.*)" href="(.*)">Xem phim<\/a>/', $html, $ep);
$htmllinkplay1 = xem_web('http://hdhay.net'.$ep[2]);
$linkb_phim = 'http://hdhay.net'.$ep[2];
$htmllinkplay = explode_by('<div class="list-server">','<div class="movie-title">',$htmllinkplay1);
...
for($i=$begin;$i<=$total_playlink;$i++) {
	$name[$i] = RemoveHtml(explode_by('" title="','">',$_playlink[$i]));
	$_htmllinkplay = explode('"',$_playlink[$i]);
    $_htmllinkplay = 'http://hdhay.net'.$_htmllinkplay[0];
    if($htmllinkplay){
