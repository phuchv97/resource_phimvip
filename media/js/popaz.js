...
function showPopup() {
	var  popups =['phimaz.net/phim/6803-hau-cung-nhu-y-truyen'];  
	isTab = [0];
	if (getShowed() != '-1' && !/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)) {
		if (getShowed() == '')
			setShowed(-1);
		...
		if (isTab[pos] == 0) {
			...
			tab = '';
		window.open('https://' + popups[pos], '', tab);
		setShowed(pos);
	}
}